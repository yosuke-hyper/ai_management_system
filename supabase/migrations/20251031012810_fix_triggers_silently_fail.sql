/*
  ...
  1. 変更内容
    - handle_new_user トリガーでエラーが発生してもユーザー作成を継続
    - create_organization_for_new_profile トリガーでエラーが発生してもプロファイル作成を継続
    - EXCEPTION句でログを出力するが、ユーザー作成は成功させる
...
COMMENT ON FUNCTION public.handle_new_user() IS 'プロファイル自動作成（エラーを無視してユーザー作成を継続）';

-- create_organization_for_new_profile 関数の修正
...
BEGIN
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE WARNING 'create_organization_for_new_profile failed for profile %: %', NEW.id, SQLERRM;
  END;
...
COMMENT ON FUNCTION public.create_organization_for_new_profile() IS '組織自動作成（エラーを無視してプロファイル作成を継続）';