/*
  ...
  1. テーブル構造の変更
    - `ai_usage_settings` を店舗ベースに変更
      - `role` カラムを削除
      - `store_id` カラムを追加
      - `organization_id` と `store_id` の複合ユニーク制約
    - `ai_usage_limits` に店舗別トラッキングを追加
      - `store_id` カラムを追加
      - 店舗ごとの月間使用回数を記録
    - `ai_usage_tracking` に店舗情報を追加（存在しない場合）
      - `store_id` カラムを追加

  2. 新規テーブル
    - `admin_override_logs` テーブル
      - 管理者による上限オーバーライドの履歴を記録
      - override_type: 'increase_limit', 'reset_usage'
      - 実行した管理者とタイムスタンプを保存

  3. RPC関数
    - `get_store_usage_status` - 店舗ごとの使用状況を取得
    - `admin_override_store_limit` - 管理者が店舗の上限を一時的に増加
...
-- 新しい複合ユニーク制約を追加（organization_id, store_id, month）
ALTER TABLE ai_usage_limits
...
-- ai_usage_tracking に店舗情報を紐付け（ユーザーの所属店舗を設定）
UPDATE ai_usage_tracking at
SET store_id = (
  SELECT om.store_id
  FROM organization_members om
  WHERE om.user_id = at.user_id
  AND om.organization_id = at.organization_id
  LIMIT 1
)
WHERE at.store_id IS NULL
